...
void Token::init()
{
    ...
    fn_map[ "extract" ] = 2;  // x(Y,i)      extract element 'i' from vector 'Y'
...
bool Token::bool_element(const int i) const
{
  ...
  if ( ttype == STRING_VECTOR ) return bvec[i];
  if ( ttype == STRING ) return bval;
  return false;  
}
...
std::vector<std::string> Token::as_string_vector() const
{
  ...
  Helper::halt( "as_string_vector() not defined" );
  return ans;
}
...
Token TokenFunctions::fn_vec_extract( const Token & tok , const Token & idx ) const
{
  ...
  if ( i < 0 || i >= tok.size() ) return Token();
  ...
  if      ( ttype == Token::INT_VECTOR )    return Token( tok.int_element(i) ); 
  else if ( ttype == Token::FLOAT_VECTOR )  return Token( tok.float_element(i) );
  else if ( ttype == Token::STRING_VECTOR ) return Token( tok.string_element(i) );
  else if ( ttype == Token::BOOL_VECTOR )   return Token( tok.bool_element(i) );
