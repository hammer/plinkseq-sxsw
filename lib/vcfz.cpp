...
bool VCFZ::open()
{
    ...
    else file = bgzf_open( filename.c_str(), "w" );
...
{    
	...
	int64_t idx64 = atol( argv[1] );
	int64_t ridx = bgzf_seek( file , idx64 , SEEK_SET );	

	if ( ridx == -1 ) { std::cerr << "PROBLEM SEEKING THIS RECORD....\n"; exit(1); } 

	std::vector<char> line;
	
	bool okay = bgzf_read_line( file , &line );
	
        if ( okay ) 
	    std::cout << "OK \n\n[" << &(line[0]) << "]\n\n";
	else
	{ std::cout << "Problem reading after SEEK\n"; exit(1); }
...
// while ( 1 ) 
// {	
// const int64_t idx = bgzf_tell( file );					
// 	bool okay = bgzf_read_line( file , &line );	
// 	if ( ! okay ) std::cout << "Hmmm, not OKAY rading\n";		
//         std::cout << ++lines_read << "\t" << idx << "\t" << &(line[0]) << "\n";
// }   
// bgzf_close( file );



void VCFZ::read_header( VarDBase * v )
{
